...
PluginCore::PluginCore()
{
    ...
    addSupportedIOCombination({ kCFStereo, kCFStereo }); // <-- Leave this line in!!!
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
    ...
    double outL, outR, outL_t, outR_t, sum, diff;
    //double volumelinDZ = dz_volume.smooth(volumelin);
    //double width_rad_dz = dz_width.smooth(theta);
    //double balance_dz = dz_balance.smooth(balance);

    sum = inL + inR;
    diff = inL - inR;
    
    outL_t = costh * sum + sinth * diff;
    outR_t = costh * sum - sinth * diff;
    outL = volumelin * ((2.0 - realbal) * outL_t);
    outR = volumelin * (realbal * outR_t);
    //outL = volumelinDZ * (((cos(width_rad_dz) * sum) + (sin(width_rad_dz) * diff)) * (2 - balance_dz));
    //outR = volumelinDZ * (((cos(width_rad_dz) * sum) - (sin(width_rad_dz) * diff)) * balance_dz);
...
bool PluginCore::postUpdatePluginParameter(int32_t controlID, double controlValue, ParameterUpdateInfo& paramInfo)
{
    ...
    switch(controlID)
    {
        ...
        {
            volumelin = pow(10, volume / 20);
            return true;    /// handled
        }
        case controlID::width:
        {
            //double width_dz = dz_width.smooth(width);
            theta = (width) * M_PI_2 / 200.0;
            costh = cos(theta);
            sinth = sin(theta);
            return true;    // handled
        }
        case controlID::balance:
        {
            //double balance_dz = dz_balance.smooth(balance);
            realbal = balance / 100.0 + 1.0;
            return true;
        }
        default:
...
bool PluginCore::initPluginParameters()
{
    ...
    piParam = new PluginParameter(controlID::width, "Width", "%", controlVariableType::kDouble, 0.000000, 200.000000, 100.000000, taper::kLinearTaper);
    piParam->setBoundVariable(&width, boundVariableType::kDouble);
    addPluginParameter(piParam);

    piParam = new PluginParameter(controlID::balance, "Balance", "%", controlVariableType::kDouble, -100.000000, 100.000000, 0.000000, taper::kLinearTaper);
    piParam->setBoundVariable(&balance, boundVariableType::kDouble);
    addPluginParameter(piParam);
    ...
    /* Log RMS VU Meter Sum*/
        piParam = new PluginParameter(controlID::vuS, "VU Sum", 5.00, 250.00, ENVELOPE_DETECT_MODE_RMS, meterCal::kLogMeter);
        piParam->setInvertedMeter(false);
        piParam->setIsProtoolsGRMeter(false);
        piParam->setBoundVariable(&vuS, boundVariableType::kFloat);
        addPluginParameter(piParam);

    /* Log RMS VU Meter Diff*/
        piParam = new PluginParameter(controlID::vuD, "VU Diff", 5.00, 250.00, ENVELOPE_DETECT_MODE_RMS, meterCal::kLogMeter);
        piParam->setInvertedMeter(false);
        piParam->setIsProtoolsGRMeter(false);
